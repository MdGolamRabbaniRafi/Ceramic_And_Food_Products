...
export class DiscountController {
    ...
    @Get('/:id')
    async getDiscountById(@Param('id', ParseIntPipe) id: number): Promise<DiscountEntity | null> {
      return await this.discountService.getDiscountById(id);
    }
  
    @Post('/add')
    async createDiscount(@Body() discountData: Partial<DiscountEntity>, @Body('productId') productId: number): Promise<DiscountEntity | boolean> {
    ...
    @Delete('/delete/:id')
    async deleteDiscount(@Param('id', ParseIntPipe) id: number): Promise<boolean> {
